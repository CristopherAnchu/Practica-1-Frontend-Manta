...
export class McpClientService implements OnModuleInit {
  ...
  async listTools(): Promise<any[]> {
    try {
      ...
      const response = await this.client.post('/mcp/tools/list', jsonRpcRequest);
  ...
  async callTool(name: string, args: any): Promise<any> {
    try {
      ...
      const response = await this.client.post('/mcp/tools/call', jsonRpcRequest);

      if (response.data.error) {
        throw new Error(`MCP Tool error: ${response.data.error.message}`);
      }

      this.logger.log(`✅ Tool ejecutada exitosamente: ${name}`);
